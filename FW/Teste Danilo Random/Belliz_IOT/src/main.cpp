...
void callback(char* topic, byte* payload, unsigned int length) {
 ...
 // }
  ...
  if ((char)payload[0] == '1') {
    ...
    Serial.println("ON");
    comand_upload = true;
    digitalWrite(GPIO_LED_VM, HIGH);  // Turn the LED off by making the voltage HIGH
    delay(1000);
    digitalWrite(GPIO_LED_VM, LOW);  // Turn the LED off by making the voltage HIGH
    delay(1000);
    digitalWrite(GPIO_LED_VM, HIGH);  // Turn the LED off by making the voltage HIGH
    delay(1000);
    digitalWrite(GPIO_LED_VM, LOW);  // Turn the LED off by making the voltage HIGH
...
void ambientTempHumidity(){
    ...
    humidity_room = random(100);
    temperature_room = random(40);
...
void sendData(){
 ...
 if(device == 0){
    ...
    snprintf(messages, 140, "{\"FW\":%.2f,\"A\":%.2f,\"RPM\":%d,\"TE\":%.2f,\"EPC\":%d,\"HR\":%.2f,\"TR\":%.2f,\"FIRE\":%d}", last_version_fw, current, rpm, temperature, epochTime, humidity_room, temperature_room, fireDetector);
 ...
 if(device == 1){
    ...
    snprintf(messages, 125, "{\"FW\":%.2f,\"A\":%.2f,\"RPM\":%d,\"TE\":%.2f,\"EPC\":%d}", last_version_fw, current, rpm, temperature, epochTime);
    client.publish(outTopic, messages);
 }
